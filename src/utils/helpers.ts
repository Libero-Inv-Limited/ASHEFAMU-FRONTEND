...
export const calculateBoundingBox = (
  ...
  const latDelta = distanceInKilometers * degreesPerPixel;
  const lonDelta =
    (distanceInKilometers * degreesPerPixel) /
    Math.cos((centerLatitude * Math.PI) / 180);

  const startLatitude = centerLatitude - latDelta;
  const stopLatitude = centerLatitude + latDelta;
  const startLongitude = centerLongitude - lonDelta;
  const stopLongitude = centerLongitude + lonDelta;
