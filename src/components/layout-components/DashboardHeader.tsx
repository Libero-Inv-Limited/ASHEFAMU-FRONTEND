...
import { DARK, LIGHT_GRAY, LIGHT_GREEN, TEXT_DARK_GRAY, TEXT_GRAY } from "../../utils/color"
              ...
              isEditing ? (
                  <HStack>
                    <IconButton 
                      ...
                      icon={<Icon as={BsArrowLeft} fontSize={"xl"} color={TEXT_DARK_GRAY} />}
                    />
                    <Heading noOfLines={1} fontSize={["xl", "xl", "2xl"]} color={DARK} textTransform={"uppercase"}>Editing <span style={{ color: TEXT_GRAY }}> {decodeSlug(isEditing)}</span> </Heading>
                  </HStack>
                ) : (
        ...
        <Menu>
          <MenuButton>
            <HStack alignItems={"center"}>
              <Stack spacing={0} textAlign={"right"} display={["none", "none", "flex", 'flex']}>
                ...
                <Text fontSize={"sm"} color={TEXT_DARK_GRAY}>{user?.user.email}</Text>
              </Stack>
          ...
          <MenuList pt={0}>
            <Stack spacing={0} bg={LIGHT_GRAY} p={3}>
              ...
              <Text fontSize={"sm"} fontFamily={"rubik"} color={"primary.600"}>{user.user.userRole?.roleDetails?.name}</Text>
            </Stack>
            <MenuItem _hover={{ bg: LIGHT_GREEN }} onClick={() => navigate(ROUTES.EDIT_PROFILE)} py={3} >
              <Text fontSize={"sm"} color={DARK}>Edit profile</Text>
            </MenuItem>

            <MenuItem _hover={{ bg: LIGHT_GREEN }} onClick={openLogoutModal} py={3}>
              <Text fontSize={"sm"} color={DARK}>Logout</Text>
            </MenuItem>
          </MenuList>
        </Menu>
      </HStack>
