...
const SecondarySidebarItem: React.FC<SecondarySidebarItemProps> = ({isElementIcon, hasStroke, ...prop}) => {
  ...
  return (
    <HStack bg={isActive ? LIGHT_BLUE : ""} rounded={"sm"} to={prop.link.replace(":name", param.name!)} _hover={{ bg: !isActive && "gray.100" }} color={isActive ? "primary.500" : TEXT_DARK_GRAY } fill={isActive ? "primary.500" : TEXT_DARK_GRAY } stroke={hasStroke ? isActive ? "primary.500" : TEXT_DARK_GRAY : undefined} spacing={2} as={Link} alignItems={"center"} p={3} h={"40px"}>
      { isElementIcon ? prop.icon :
