    ...
    <HStack>
      ...
      <HStack>
        <Select
          ...
          fontSize={"sm"}
          color={TEXT_DARK}
          onChange={({ target }) => onChangePage(+target.value)}
        >
          {new Array(noOfPages).fill(0).map((_, index) => (
            <option value={index + 1} selected={index + 1 === page} key={index}>
              {index + 1}
            </option>
          ))}
        </Select>
  ...
  return (
    ...
    >
      <HStack>
        ...
        <Text whiteSpace={"nowrap"} fontSize={"sm"} color={TEXT_DARK}>
          {page} - {totalInView} of {count} entries
        </Text>
      </HStack>
