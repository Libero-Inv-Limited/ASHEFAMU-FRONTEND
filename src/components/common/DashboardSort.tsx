...
const DashboardSort: React.FC<DashboardSortProps> = ({ onCancel }) => {
  const dashboardCards = useAppSelector(
    (state) => state.dataStore.dashboardCards
  );
  const [tempState, setTempState] = useState<DashboardCardType[]>(
    dashboardCards.filter((card) => card.visibility)
  );
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    const item = tempState.find(
      (item) => item.name === draggableId.split("-")[1]
    );
    if (!item) return;
  ...
  return (
    <ModalContent>
      ...
      <ModalFooter as={HStack} spacing={3}>
        <CustomButton variant={"outline"} colorScheme="gray" onClick={onCancel}>
          Cancel
        </CustomButton>
        <CustomButton
          isLoading={isSaving}
