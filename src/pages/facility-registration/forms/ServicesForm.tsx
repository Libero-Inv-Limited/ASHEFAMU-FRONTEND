...
const ServicesForm: React.FC<ServicesFormProps> = ({
  ...
  const defaultValues = allData["SERVICES"] || {};
  ...
  const { control, setValue, getValues, trigger } = useForm<ServiceData | any>({
    mode: "onSubmit",
  });
  const handleSaveInfo = async () => {
    ...
    try {
      ...
      const data: any = {
        ...
        waterSources: getValues("waterSources").map((item: any) => ({
          water_source: item.label,
        })),
        ...
        wasteDisposalMethods: JSON.stringify({
          human_waste: (getValues("human_waste") as any).map(
            (item: any) => item.label
          ),
          medical_waste: (getValues("medical_waste") as any).map(
            (item: any) => item.label
          ),
          refuse_disposal: (getValues("refuse_disposal") as any).map(
            (item: any) => item.label
          ),
        }),
      };
        ...
        <Grid gridTemplateColumns={"repeat(12, 1fr)"} gap={4}>
          <GridItem colSpan={[12, 12, 5]}>
            <AuthInput
              ...
              data={wasteDisposalMethods.map((item) => ({
                label: item.name,
                value: item.name,
              }))}
              isSelect
