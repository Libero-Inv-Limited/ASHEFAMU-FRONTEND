...
import {
  convertToSnakecase,
  handleConvertCSVToArray,
  readFile,
} from "../../../utils/helpers";
import StaffCompliment from "../../../components/common/StaffCompliment";
import {
  executeAddNonProfessional,
  executeAddProfessional,
} from "../../../apis/facility";
...
const FacilityStaffForm: React.FC<FacilityStaffFormProps> = ({
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<{ staff_list: File | undefined }>({ mode: "onSubmit" });
  const {
    control,
  ...
  const handleSaveInfo = async () => {
    ...
    try {
      ...
      const resultNonProf = await executeAddNonProfessional(
        nonProfPayload,
        token!
      );
      console.log("errorsssssssss");
      if (resultNonProf.status === "error")
        throw new Error(resultNonProf.message);
    ...
    <Stack spacing={14}>
      <Stack spacing={6}>
        ...
        <CustomButton
          ...
          leftIcon={<Icon as={FiDownload} fontSize={"xl"} />}
          w={"fit-content"}
        >
          Download template
        </CustomButton>
      ...
      <Stack spacing={6}>
        <FormTitle>
          UPLOAD COMPLETED TEMPLATE (CSV, 10MB maximum size)
        </FormTitle>
