...
import { Heading, Icon, Stack, useDisclosure, useToast } from "@chakra-ui/react"
...
const CreateIntent: React.FC<CreateIntentProps> = () => {
  const { control, register, formState: { errors }, getValues, trigger, watch, setValue, setError } = useForm<IntentType>({
    mode: "onSubmit"
  })
  ...
  const { loadingText, startLoadingText, stopLoadingText } = useWaitingText(["Checking validity", "Submitting", "Proccessing"], 3000)
  const { isFetching, facilityCategory, sectorCategory } = useFetchFacilityData()
  ...
  const handleSave = async () => {
    try {
      ...
      formData.append("letter_of_intent", data.letter_of_intent, data.letter_of_intent.name)
    ...
    <DashboardLayout>
      <Stack rounded={"md"} p={9} spacing={6} bg={"white"} maxW={647}>
        ...
        <Stack spacing={4}>
          ...
          <CustomButton
            onClick={handleSave} alignSelf={"flex-end"}
            w={"fit-content"} isLoading={isLoading}
            loadingText={loadingText}
            rightIcon={<Icon fontSize={"xl"} as={HiArrowNarrowRight} />}>Save & proceed</CustomButton>
