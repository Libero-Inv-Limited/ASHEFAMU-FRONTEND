...
const FacilityProfile: React.FC<FacilityProfileProps> = () => {
  ...
  const printImage = () => {
    ...
    printWindow.document.write(
      '<html><body style="margin:0;"><img src="' +
        currentFacility?.qr_code +
        '" alt="Image to Print"></body></html>'
    );
  ...
  return (
    <DashboardLayout>
        ...
        >
          ...
          <Stack>
            <HStack>
              ...
              />
              <Link
                href={`tel:${currentFacility?.facility_phone}`}
                fontSize={"sm"}
                color={TEXT_DARK_GRAY}
              >
                {currentFacility?.facility_phone}
              </Link>
            </HStack>
          </Stack>
              ...
              >
                ...
                <HStack>
                  ...
                  >
                    ...
                    >{`${currentFacility?.createdBy?.firstname} ${currentFacility?.createdBy?.lastname}`}</span>
                  </Text>
                  <Icon color={TEXT_DARK_GRAY} as={BsDot} fontSize={"xl"} />
                  <Text flex={1} color={TEXT_DARK_GRAY} fontSize={"sm"}>
                    {formatDate(currentFacility.created_at)}
                  </Text>
                </HStack>
              </Stack>
              <CustomButton
        ...
        <Stack spacing={6} p={6} bg={"white"} rounded={"md"} maxW={250}>
          ...
          <HStack>
            <CustomButton
              ...
              borderColor={"#C9CFD8"}
              fontSize={"sm"}
              leftIcon={<Icon fontSize={"xl"} as={MdLocalPrintshop} />}
              variant={"outline"}
              size={"sm"}
            >
              Print
            ...
            <CustomButton
              ...
              borderColor={"#C9CFD8"}
              fontSize={"sm"}
              leftIcon={<Icon fontSize={"xl"} as={FiDownload} />}
              variant={"outline"}
              size={"sm"}
            >
              Download
      ...
      <FacilityUpgrade isOpen={isOpen} onClose={onClose} />
    </DashboardLayout>
  );
};
